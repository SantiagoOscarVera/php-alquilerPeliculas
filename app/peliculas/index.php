...
session_start();    /* como estamos trabajando mensajes de sesion forzosamente necesitamos declarar */
        ...
        <tbody>
            <?php while ($row_peliculas = $peliculas->fetch_assoc()) { ?>
                <tr>
                    ...
                    <td> <img src="<?= $dir . $row_peliculas['id'] . '.jpg?n=' . time(); ?>" width='100' alt="Imagen pelicula"> </td> <!-- esto añade la imagen. ?n=' . time(); esto hace que se actualice la imagen al instante y no queden cosas en el cache -->
                    <td>
...
/*         editaModal.addEventListener('hide.bs.modal', event => {
        ...
        eliminaModal.addEventListener('shown.bs.modal', event => {
            let button = event.relatedTarget /* necesitamos saber a que icono se le dio click */
            let id = button.getAttribute('data-bs-id') /* ya con el boton detectado llamo al atributo data-bs-id que ya lo defini en la tabla*/
            eliminaModal.querySelector('.modal-footer #id').value = id /* al obtener el id, buscamos el id y le pasamos en su valor la variable id para que pueda saber que registro puede eliminar */
        })
    </script>
